...
from metrics_styleinv import metric_utils
...
def subprocess_fn(rank, args, temp_dir):
    ...
    random.seed(random_seed * args.num_gpus + rank)
    np.random.seed(random_seed * args.num_gpus + rank)
    torch.manual_seed(random_seed * args.num_gpus + rank)
...
def calc_metrics_for_dataset(ctx, metrics, real_data_path, fake_data_path, mirror, resolution, gpus, verbose, use_cache: bool, num_runs: int, seed: int):
    ...
    args.dataset_kwargs = dnnlib.EasyDict(
        ...
        return_vid=True,
        sampling_cfg=dnnlib.EasyDict(type='N/A') # just indicate a type other from 'random' or 'beta'
    )
    ...
    args.gen_dataset_kwargs = dnnlib.EasyDict(
        ...
        resolution=resolution,
        return_vid=True,
        sampling_cfg=dnnlib.EasyDict(type='N/A')
    )
...
@click.command()
@click.pass_context
# Here we modify the paper FID to be fid50k, not using all frames of a dataset to compute FID
@click.option('--metrics', help='Comma-separated list or "none"', type=CommaSeparatedList(), default='fvd2048_16f,fid50k', show_default=True)
...
@click.option('--seed', help='Seed Number', type=int, default=1141514, metavar='INT', show_default=True)

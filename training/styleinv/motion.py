...
from training.networks_sgv import MappingNetwork, FullyConnectedLayer, EqLRConv1d
...
class MotionMappingNetwork(torch.nn.Module):
    def __init__(self, cfg: DictConfig):
        ...
        assert self.cfg.motion.gen_strategy in ["autoregressive", "conv", "abl_no_ffa"], f"Unknown generation strategy: {self.cfg.motion.gen_strategy}"
        ...
        if self.cfg.motion.gen_strategy == 'autoregressive':
            ...
            """
            # Now we use a left-side 1D convolution to ensure that all anchor points are only affected by the left ones
            # Combining that the first frames is always provided a constant vector
            # This ensures that the embedding of t=0 does not change.
            ks = self.cfg.motion.kernel_size
            self.frame_0_const = torch.nn.Parameter(torch.randn([self.cfg.motion.z_dim]))
            self.conv = nn.Sequential(
                EqLRConv1d(self.cfg.motion.z_dim + self.cfg.c_dim, self.cfg.motion.z_dim, ks, padding=(ks//2), activation='lrelu', lr_multiplier=0.01, mask_right=True),
                EqLRConv1d(self.cfg.motion.z_dim, self.cfg.motion.v_dim, ks, padding=(ks//2), activation='lrelu', lr_multiplier=0.01, mask_right=True),
            )
            #self.num_additional_codes = (self.cfg.motion.kernel_size - 1) * 2
            self.num_additional_codes = 0
        elif self.cfg.motion.gen_strategy == 'abl_no_ffa': # follow stylegan-v implementation
            ks = self.cfg.motion.kernel_size
            self.conv = nn.Sequential(
                EqLRConv1d(self.cfg.motion.z_dim + self.cfg.c_dim, self.cfg.motion.z_dim, ks, padding=0, activation='lrelu', lr_multiplier=0.01, mask_right=False),
                EqLRConv1d(self.cfg.motion.z_dim, self.cfg.motion.v_dim, ks, padding=0, activation='lrelu', lr_multiplier=0.01, mask_right=False),
            )
            self.num_additional_codes = (self.cfg.motion.kernel_size - 1) * 2
        else:
    ...
    def generate_motion_u_codes(self, c: torch.Tensor, t: torch.Tensor, motion_z: torch.Tensor=None) -> Dict:
        ...
        if self.cfg.motion.gen_strategy != 'abl_no_ffa':
            motion_z_0 = self.frame_0_const.unsqueeze(0).unsqueeze(0).repeat(batch_size, 1, 1)
            motion_z = torch.cat([motion_z_0, motion_z[:, 1:, :]], dim=1)
        ...
        elif self.cfg.motion.gen_strategy in ['conv', 'abl_no_ffa']:
        ...
        left_idx = (t / self.cfg.motion.motion_z_distance).floor().long() # [batch_size, num_frames], stores the indexes of anchor points (1st, 2nd anchor)
        ...
        t_left = t - t % self.cfg.motion.motion_z_distance # [batch_size, num_frames], in unit of frames (1st, 2nd frame)
        ...
        motion_u = motion_u.view(batch_size * num_frames, motion_u.shape[2]).to(torch.float32) # [batch_size * num_frames, motion.z_dim] -> u_t = lerp(u_l, u_r, t)

        # Save the results into the output dict
        out['motion_u_left'] = motion_u_left # [batch_size, num_frames, motion.z_dim] -> all u_l
        out['motion_u_right'] = motion_u_right # [batch_size, num_frames, motion.z_dim] -> all u_r
        out['t_left'] = t_left # [batch_size, num_frames] -> left anchor point frame index
        out['t_right'] = t_right # [batch_size, num_frames] -> right anchor point frame index
        out['interp_weights'] = interp_weights # [batch_size, num_frames, 1] -> (t % delta) / delta \in [0, 1]
        out['motion_u'] = motion_u # [batch_size * num_frames, motion.z_dim] -> interpolated u_t
    ...
    def forward(self, c: torch.Tensor, t: torch.Tensor, motion_z=None) -> Dict:
...
class AlignedTimeEncoder(nn.Module):
    ...
    def forward(self, t: torch.Tensor, motion_u_left: torch.Tensor, motion_u_right: torch.Tensor, interp_weights: torch.Tensor, t_left: torch.Tensor, t_right: torch.Tensor):
        ...
        # omega_l = periods * self.freqs, t \in {0, 1, ..., max_num_frames}
